...
int main(int argc, char *argv[])
{
  ...
  char tname[100];
  ofstream probfile;
  sprintf(tname, "pBound_VsEventTime_dt%g_NB%d_ka%g_D%g_kb%g_sig%g.dat",deltat, NB, ka, Dtot,kb, sigma);
  probfile.open(tname);
  //  int Nit1=Nit+1;
  

//   int *pBoundHist=new int[Nit1];
//   for(j=0;j<Nit1;j++){
//     pBoundHist[j]=0;
//   }
  ...
  for(rep=0;rep<Nrep;rep++){
	...
	while(nevent<MaxEvent){
	  ...
	  if(aBound==1){
		...
		if(pDissoc>rnum){
		  cout <<" DISSOCIATION: "<<dit<<' '<<rep<<endl;
		  probfile <<dit*deltat<<' '<<rep<<' '<<currBound/(1.0*dit)<<endl;
	  ...
	  if(ncross>0){
		...
		for(i=0;i<ncross;i++){
		  ...
		  if(probAssoc[i]>rnum){
			/*PERFORM ASSOCIATION*/
			cout <<" ASSOCIATION ! "<<aPartner<<" iter: "<<dit<<' '<<rep<<" currbound: "<<currBound/(1.0*dit)<<endl;
			probfile <<dit*deltat<<' '<<rep<<' '<<currBound/(1.0*dit)<<endl;
			nevent++;
			aBound=1;
...
//   for(i=1;i<Nit+1;i++){
// 	probfile <<i*deltat<<'\t'<<pBoundHist[i]/(1.0*Nrep)<<endl;
//   }
//   probfile.close();
